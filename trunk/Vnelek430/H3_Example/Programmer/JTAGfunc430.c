...
void ResetTAP(void)
{
    ...
    // Perform fuse check
    ClrTMS();
    usDelay5(); // at least 5us low required
    SetTMS();
    ClrTMS();
    usDelay5(); // at least 5us low required
    SetTMS();
...
static void StartJtag(void)
{
	...
	ClrTST();
	usDelay800();              // delay min 800us - clr SBW controller
	SetTST();
	usDelay50(); 

	// JTAG - Spy-Bi-Wire entry sequence
	ClrRST();
	usDelay100();             // delay 100us
	ClrTST();
	usDelay5();              // delay 5us
	SetTST();
	usDelay5();              // delay 5us
...
word GetDevice(void)
{
    ...
    for (i = 50; i > 0; i--)
    {
    	// Readout JTAG Control Register (1xx, 2xx, 4xx)
        if (DR_Shift16(0x0000) & BIT9)		// Synchronized ???
        {
            word DeviceId;
            ...
            DeviceId = (DeviceId << 8) + (DeviceId >> 8); // swap bytes
            
            //! \details: For debug only
            
            // usb_buffer[0] = DeviceId>>8;
            // usb_buffer[1] = DeviceId;
...
/*===============================================================================================
  ...
  Return value :  None.
================================================================================================*/
word detectTargetL(word* DeviceId, byte* SubId)
{
    word i;
    for (i = 0; i < MAX_ENTRY_TRY; i++)
    {
    ...
    for (i = 50; i > 0; i--)
    {
        ...
        if (DR_Shift16(0x0000) & BIT9)		// Synchronized ???
        {
			...
			// Get target device type (bytes are interchanged)
            data = ReadMem(F_WORD, 0x0FF0);

			// Swap bytes
            *DeviceId = (data << 8) + (data >> 8);

			// Get terget device sub-type
			*SubId = ReadMem(F_BYTE, 0x0FFD);			
            break;
        }
        else
        {
...
//! STATUS_ERROR otherwise)
word WriteFLASHallSections(const unsigned int *data, const unsigned long *address, const unsigned long *length_of_sections, 
