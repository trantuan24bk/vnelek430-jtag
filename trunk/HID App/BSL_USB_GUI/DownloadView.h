			...
			while (true)
			{
				...
				if (line->Substring(0, 1) == "@")
				{
					...
					if (usbIndex > START_INDEX)
					{
						...
						if(TX_TransmitData(usbDataFrame, receiveBuffer, CMD_WRITE) != STATUS_SUCCESS)
						{
							return;
						}
				...
				else if (line == "q")
				{
					...
					if(TX_TransmitData(usbDataFrame, receiveBuffer, CMD_WRITE_END) != STATUS_SUCCESS)
					{						
						return ;
					}
...
private: System::Void btnProgram_Click(System::Object^  sender, System::EventArgs^  e) 
		 {
			 //unsigned char receiveData[100];
			 //unsigned char transmitData[100] = "hello";
			 //unsigned int size;
			 //LPTSTR pChar;
			 //
			 ...
			 //{
				//// Send data to device
				//BSL_USB_SendData(transmitData, 5);

				//// Receive data from device
				//size = BSL_USB_ReceiveData((unsigned char*)receiveData);

				//// Insert end of string: 'null'
				//receiveData[size] = 0; 

				//// Update status
				//textBoxLog->Text = "Received " + size + " bytes\n";

				//pChar = (char*)receiveData;
				//textBoxLog->Text += gcnew String(pChar);
         ...
         * Return       : None.
         * ========================================================================= */
        int TX_TransmitData(byte data[], BYTE receiveBuffer[], byte command)
        {
            int length, address;
			unsigned int i;
			...
			// The programmer is not respoding..
			if(i == 10)
			{
...
private: System::Void btnEraseAll_Click(System::Object^  sender, System::EventArgs^  e) 
		 {
			...
			TX_TransmitData(usbData, receiveBuffer, CMD_ERASE);
...
private: System::Void btnDetectDevice_Click(System::Object^  sender, System::EventArgs^  e) 
		 {
			...
			// Send erase target command to programmer
			TX_TransmitData(usbData, receiveBuffer, CMD_DETECT);
